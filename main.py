...
class ResourceManager:
    ...
    async def process_with_limit(self, coroutine):
        async with self.semaphore:
            return await coroutine

# Part 2: Video Processing Functions
...
async def compress_video(input_file: str, settings: dict) -> str:
    try:
...
async def process_final_video(input_file: str, user_data: dict) -> str:
    output_file = input_file
    temp_files = []
    
    try:
        # Process streams if needed
        if user_data['selected_streams']:
            stream_output = await process_video(
                output_file,
                user_data['selected_streams'],
                len(user_data['streams'])
            )
            if output_file != input_file:
                temp_files.append(output_file)
            output_file = stream_output
        
        # Compress if settings are specified
        if any(user_data['compression_settings'].values()):
            compress_output = await compress_video(
                output_file,
                user_data['compression_settings']
            )
            if output_file != input_file:
                temp_files.append(output_file)
...
class VideoProcessor:
    ...
    async def process(self, input_file: str, user_data: dict) -> str:
        return await self.resource_manager.process_with_limit(
            process_final_video(input_file, user_data)
        )
    ...
    async def validate_video(self, file_path: str) -> bool:
        try:
            info = await self.get_video_info(file_path)
            return any(s['codec_type'] == 'video' for s in info.get('streams', []))
        except Exception:
            return False

# Part 3: UI Components and Button Creators
...
def create_stream_buttons(streams: list, selected_streams: Set[int]) -> list:
    ...
    buttons.append([
        InlineKeyboardButton("✅ 𝗣𝗿𝗼𝗰𝗲𝘀𝘀", callback_data="continue"),
        InlineKeyboardButton("❌ 𝗖𝗮𝗻𝗰𝗲𝗹", callback_data="cancel")
    ])
    
    return buttons

def create_compression_buttons() -> list:
    buttons = [
        [InlineKeyboardButton("🎯 Resolution", callback_data="comp_resolution")],
        [
        ...
        [InlineKeyboardButton("⚙️ Quality (CRF)", callback_data="comp_crf")],
        [
            InlineKeyboardButton("High", callback_data="crf_18"),
            InlineKeyboardButton("Medium", callback_data="crf_23"),
            InlineKeyboardButton("Low", callback_data="crf_28")
        ],
        [InlineKeyboardButton("⚡ Encoding Speed", callback_data="comp_preset")],
        [
...
def create_initial_options() -> list:
    buttons = [
...
async def update_compression_menu(callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    settings = user_data[user_id]['compression_settings']
    
    # Create status indicators
    resolution_status = settings['resolution'] or 'Original'
    quality_status = {
        18: "High",
        23: "Medium",
        28: "Low"
    }.get(settings['crf'], "Medium")
    
    preset_status = {
        'fast': "Fast",
        'medium': "Medium",
        'slow': "Slow"
    }.get(settings['preset'], "Medium")
    
    text = (
        "**🔄 Compression Settings**\n\n"
        f"• Resolution: {resolution_status}\n"
        f"• Quality: {quality_status}\n"
        f"• Speed: {preset_status}\n\n"
        "Select options or continue:"
    )
    
    buttons = create_compression_buttons()
...
class MessageFormatter:
    ...
    @staticmethod
    def get_stream_selection_message() -> str:
        return (
    ...
    @staticmethod
    def get_upload_options_message() -> str:
        return (
    ...
    @staticmethod
    def format_file_caption(filename: str) -> str:
        return f"**{filename}**\n"


# Part 4: Message Handlers and Bot Logic

@app.on_message(filters.command("start"))
async def start_command(client, message: Message):
    await message.reply_text(MessageFormatter.get_start_message())
...
async def handle_video(client, message: Message):
    try:
        ...
        buttons = create_initial_options()
    ...
    if user_id in user_data and user_data[user_id].get('awaiting_rename'):
        ...
        if message.text == "/cancel":
            ...
            await client.edit_message_text(
                ...
                text=MessageFormatter.get_upload_options_message(),
                reply_markup=InlineKeyboardMarkup(buttons)
            )
...
async def handle_callback(client, callback_query: CallbackQuery):
    ...
    if user_id not in user_data and callback_query.data not in ['header', 'remove_streams', 'compress_video']:
    ...
    try:
        if data == "remove_streams":
            buttons = create_stream_buttons(user['streams'], set())
            await callback_query.message.edit_text(
                MessageFormatter.get_stream_selection_message(),
                reply_markup=InlineKeyboardMarkup(buttons)
            )
            ...
            user['compression_settings']['resolution'] = resolution
            await update_compression_menu(callback_query)
            
        elif data.startswith("crf_"):
            crf = int(data.replace("crf_", ""))
            user['compression_settings']['crf'] = crf
            await update_compression_menu(callback_query)
            
        elif data.startswith("preset_"):
            preset = data.replace("preset_", "")
            user['compression_settings']['preset'] = preset
            await update_compression_menu(callback_query)
            
        elif data in ["continue", "comp_continue"]:
            buttons = create_rename_buttons()
            await callback_query.message.edit_text(
            ...
            status_msg = await callback_query.message.edit_text(
                MessageFormatter.get_processing_message()
            )
            
            try:
                output_file = await process_final_video(user['file_path'], user)
                ...
                if os.path.exists(output_file):
                    ...
                    caption = MessageFormatter.format_file_caption(filename)
                ...
                await FileManager.cleanup_files([user['file_path'], output_file])
