...
async def handle_video(client, message: Message):
    try:
        ...
        user_data[message.from_user.id] = {
            ...
            'new_filename': None,
            'selection_time': datetime.now()  # Add this line
        }
        ...
        await status_msg.edit_text(
            ...
            "⬜️ = Keep stream\n"
            "☑️ = Remove stream\n\n"
            "⏰ You have 3 minutes to make your selection.\n"
            "_Select all streams you want to remove and press Process._",
            reply_markup=InlineKeyboardMarkup(buttons)
        )

        # Schedule timeout check
        asyncio.create_task(check_timeout(message.chat.id, message.from_user.id, status_msg.id))
...
async def check_timeout(chat_id, user_id, message_id):
    await asyncio.sleep(180)  # 3 minutes
    if user_id in user_data:
        selection_time = user_data[user_id].get('selection_time')
        if selection_time and (datetime.now() - selection_time) >= timedelta(minutes=3):
            try:
                # Clean up files
                ...
                await app.edit_message_text(
                    ...
                    text="⏰ **Selection timeout!**\n\nThe 3-minute selection period has expired. Please send your video again."
                )
            except Exception as e:
...
# Modify the callback handler to check for timeout
